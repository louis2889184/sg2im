...
from sg2im.data.vg import SequenceTransformerVgSceneGraphDataset, vg_collate_fn
from sg2im.discriminators import PatchDiscriminator, AcCropDiscriminator
from sg2im.losses import get_gan_losses
from sg2im.metrics import jaccard
from sg2im.model import TransformerSg2ImModel
from sg2im.utils import int_tuple, float_tuple, str_tuple
from sg2im.utils import timeit, bool_flag, LossManager

from transformers import AutoTokenizer, BertTokenizer, EncoderDecoderModel, EncoderDecoderConfig
...
def build_model(args, vocab):
  if args.checkpoint_start_from is not None:
    ...
    model = TransformerSg2ImModel(**kwargs)
...
def build_vg_dsets(args, tokenizer):
  ...
  train_dset = SequenceTransformerVgSceneGraphDataset(**dset_kwargs, tokenizer=tokenizer)
  iter_per_epoch = len(train_dset) // args.batch_size
  print('There are %d iterations per epoch' % iter_per_epoch)

  dset_kwargs['h5_path'] = args.val_h5
  del dset_kwargs['max_samples']
  val_dset = SequenceTransformerVgSceneGraphDataset(**dset_kwargs, tokenizer=tokenizer)
...
def build_loaders(args, tokenizer):
  if args.dataset == 'vg':
    vocab, train_dset, val_dset = build_vg_dsets(args, tokenizer)
    collate_fn = None
...
def modify_network(model, tokenizer):
  # Encoder
  # word_embedding
  old_embedding = model.encoder.embeddings.word_embeddings

  old_vocab_size = old_embedding.weight.shape[0]
  vocab_size = len(tokenizer.get_vocab())
  dim = old_embedding.weight.shape[1]
  new_encoder_word_embeddings = nn.Embedding(
    vocab_size, 
    dim, 
    padding_idx=0
  )
  # partially load the weight
  new_encoder_word_embeddings.weight.data[:old_vocab_size, :] = old_embedding.weight.data
  model.encoder.embeddings.word_embeddings = new_encoder_word_embeddings

  # position_embedding
  old_embedding = model.encoder.embeddings.position_embeddings
  new_encoder_position_embeddings = nn.Embedding(
    32 * 32, 
    dim, 
  )
  # partially load the weight
  # 1024 = 512 * 2, so copy the original weight to the first half 
  # and the second half of new embedding 
  new_encoder_position_embeddings.weight.data[:old_embedding.weight.shape[0], :] = old_embedding.weight.data
  new_encoder_position_embeddings.weight.data[old_embedding.weight.shape[0]:, :] = old_embedding.weight.data
  model.encoder.embeddings.position_embeddings = new_encoder_position_embeddings

  # Decoder
  # word_embedding
  model.decoder.bert.embeddings.word_embeddings = nn.Embedding(
    vocab_size, 
    dim, 
    padding_idx=0
  )

  # tie the paramter to encoder's
  model.decoder.bert.embeddings.word_embeddings.weight.data = \
    model.encoder.embeddings.word_embeddings.weight.data

  # position_embedding
  model.decoder.bert.embeddings.position_embeddings = nn.Embedding(
    32 * 32, 
    dim, 
  )
 
  model.decoder.bert.embeddings.position_embeddings.weight.data = \
    model.encoder.embeddings.position_embeddings.weight.data

  old_bias = model.decoder.cls.predictions.decoder.bias
  model.decoder.cls.predictions.decoder = nn.Linear(dim, vocab_size)

  model.decoder.cls.predictions.decoder.weight.data = \
    model.encoder.embeddings.word_embeddings.weight.data

  model.decoder.cls.predictions.decoder.bias.data[:old_vocab_size] = old_bias.data
